...
OFFICIAL_NEWS_METADATA_INSTRUCTION = """
メタデータ分類要件（必ず遵守）:
1. title（記事タイトル）は分類判断の極めて重要な情報です。タイトルに「古戦場」「これからのグランブルーファンタジー」「アップデート」「キャラクター」「武器」「召喚石」等の明確な語がある場合は本文と合わせて優先的な分類根拠としてください。ただしタイトルだけで本文と矛盾する分類を無理に行わないでください。
2. element（属性）は以下のいずれか1つのみを選択してください:
   ["火", "水", "土", "風", "光", "闇", "全属性", "不明"]
   - 特定の1属性が記事の主題 → 該当属性（火/水/土/風/光/闇）
   - 複数属性を扱う記事、またはゲーム全体・システム全般・全プレイヤー向けの総合告知（「これからの『グランブルーファンタジー』(これグラ)」等） → 原則として "全属性"
   - 「特定属性の記載がない」という理由だけで"不明"を選ばないでください。ゲーム全体向けなら必ず"全属性"を選択してください。
   - 属性が記事内容と関係なく、タイトル・本文からも本当に判断できない場合のみ → "不明"
3. category（大分類）は以下のいずれか1つのみを選択してください（「何についての記事か」という中心テーマを表します）:
   ["古戦場", "高難度", "周回", "キャラ情報", "武器情報", "召喚石情報", "アップデート情報", "初心者向け", "その他"]
   【優先順位・選定ルール】
   - 記事の主題が「古戦場」、またはタイトル・本文で「古戦場」が主対象として明示されている場合 → 最優先で "古戦場"（「古戦場の武器編成」「古戦場向けキャラ解説」「古戦場の攻略」のように古戦場が明確な主題の場合は、武器情報・キャラ情報・その他より "古戦場" を優先してください）
   - ゲーム機能追加、仕様変更、システム更新、複数アップデートをまとめた公式告知、「これグラ」等の総合告知 → 個別テーマではなく "アップデート情報" を優先
   - 高難度バトル・高難度マルチが主題 → "高難度"
   - 周回方法・周回効率・素材集めが主題 → "周回"
   - キャラクター追加・性能・調整が主題 → "キャラ情報"
   - 武器追加・性能・調整が主題 → "武器情報"
   - 召喚石追加・性能・調整が主題 → "召喚石情報"
   - 初心者向け案内が主題 → "初心者向け"
   - 上記のどれにも明確に該当しない場合のみ → "その他"
4. content_type（内容種別）は以下のいずれか1つのみを選択してください（「どのような形式・目的の記事か」という情報形態を表します）:
   ["編成", "攻略", "解説", "検証", "比較", "評価", "ニュース", "その他"]
   - 公式運営による開催告知・追加告知・アップデート告知・キャンペーン告知等 → 原則として "ニュース"
   - 記事自体が攻略や手順解説を主目的とする場合 → "攻略"
   - 記事自体が編成解説を主目的とする場合 → "編成"
   - 武器等の性能比較を主目的とする場合 → "比較"
5. status は以下のいずれか1つのみを選択してください:
   ["最新", "参考", "古い可能性あり"]
   - "無効" やそれ以外の値は絶対に選ばないでください。
...
async def admin_daily_summary_endpoint(http_request: Request):
    ...
    try:
        ...
        # Knowledge鮮度確認情報の追加
        try:
            freshness_data = collect_knowledge_freshness_candidates()
            # FreshnessCandidateItem の dict 変換
            freshness_data["candidates"] = [c.model_dump() for c in freshness_data["candidates"]]
            summary_data["knowledge_freshness"] = freshness_data
        except Exception as e:
            print(f"Error fetching knowledge freshness for daily summary: {e}")
            # 鮮度取得に失敗しても、既存のDaily Summary自体はそのまま返す（fail-safe）
            summary_data["knowledge_freshness"] = {
                "error": "Failed to fetch knowledge freshness",
                "total_candidates": 0,
                "expired_count": 0,
                "flagged_old_count": 0,
                "candidates": []
...
def collect_knowledge_freshness_candidates() -> dict:
    collection_ref = db.collection("knowledge")
...
def get_admin_knowledge_freshness_candidates(http_request: Request):
    require_admin(http_request)
    try:
        result = collect_knowledge_freshness_candidates()
        return FreshnessCandidatesResponse(**result)
    except Exception as e:
